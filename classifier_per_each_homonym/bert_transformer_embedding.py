...
class BertTransformerSentenceEmbedding(TransformerMixin, BaseEstimator):
    ...
    def transform(self, X):
        res = []
        for text in X:
            t = self.tokenizer(text, padding=True, truncation=True, max_length=512,
...
class BertTransformerEmbedding(BertTransformerSentenceEmbedding):
    def transform(self, X):
        res = []
        for text in X:
            t = self.tokenizer(text, padding=True, truncation=True, max_length=510, add_special_tokens=True,
                               return_tensors='pt')
            with torch.no_grad():
                model_output = self.model(**{k: v.to(self.model.device) for k, v in t.items()})
            embeddings = model_output.last_hidden_state[:, 0, :]
